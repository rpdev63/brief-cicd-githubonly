...
class AdvancedEncryptionStandard(EncryptionInterface):
    ...
    def encrypt(self, message, key=0):
        ...
        if not isinstance(key, str) or len(bytes(key, encoding='utf-8')) not in (16, 24, 32):
            ...
            self.logger.error(
                "Error during the encryption of a message with DES. The key must be a string in ASCII format of length 16, 24 or 32")
            return False
    ...
    def decrypt(self, message, key=0):
        ...
        if not isinstance(key, str) or len(bytes(key, encoding='utf-8')) not in (16, 24, 32):
            ...
            self.logger.error(
                "Error during the decryption of a message with DES. The key must be an string in ASCII format of length 16, 24 or 32")
            return False
