...
class Blowfish(EncryptionInterface):
    ...
    def encrypt(self, message, key=0):
        ...
        if not isinstance(message, str):
            ...
            self.logger.error(
                "Error during the encryption of a message with Blowfish. The message must be a string")
            return False

        if not isinstance(key, str) or len(bytes(key, encoding='utf-8')) < 4 or len(bytes(key, encoding='utf-8')) > 56:

            self.logger.error(
                "Error during the encryption of a message with Blowfish. The key must be between 4 and 56 bytes long")
        ...
        encryptedMessage = b"".join(cipher.encrypt_ctr(
            bytes(message, encoding="utf-8"), encryptionCounter))
    ...
    def decrypt(self, message, key=0):
        ...
        if not isinstance(message, str):
            ...
            self.logger.error(
                "Error during the decryption of a message with Blowfish. The message must be a string")
            return False

        if not isinstance(key, str) or len(bytes(key, encoding='utf-8')) < 4 or len(bytes(key, encoding='utf-8')) > 56:

            self.logger.error(
                "Error during the decryption of a message with Blowfish. The key must be between 4 and 56 bytes long")
            return False
        ...
        decryptedMessage = b"".join(
            cipher.decrypt_ctr(message, decryptionCounter))
