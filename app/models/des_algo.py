...
class DES(EncryptionInterface):
    ...
    def encrypt(self, message, key=0):
        ...
        if not isinstance(message, str):
            ...
            self.logger.error(
                "Error during the encryption of a message with DES. The message must be a string")
            return False

        if not isinstance(key, str) or len(bytes(key, encoding='utf-8')) not in (8, 16, 24):

            self.logger.error(
                "Error during the encryption of a message with DES. The key must be a string in ASCII format of length 8, 16 or 24")
            return False

        userKey = DesKey(bytes(key, encoding='utf-8'))

        encryptedMessage = userKey.encrypt(
            bytes(message, encoding='utf-8'), padding=True)
    ...
    def decrypt(self, message, key=0):
        ...
        if not isinstance(message, str):
            ...
            self.logger.error(
                "Error during the decryption of a message with DES. The message must be a string")
            return False

        if not isinstance(key, str) or len(bytes(key, encoding='utf-8')) not in (8, 16, 24):

            self.logger.error(
                "Error during the decryption of a message with DES. The key must be an string in ASCII format of length 8, 16 or 24")
            return False
