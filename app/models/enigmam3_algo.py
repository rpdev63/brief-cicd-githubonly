    ...
    def encrypt(self, message, key=0):
       ...
       - a list of 10 tuples of two chars (at most) for the plugboard settings"""
        ...
        if not isinstance(message, str):
            ...
            self.logger.error(
                "Error during the encryption of a message with Enigma. The message must be a string")
            return False

        if not self.checkKeyFormat(key):

            self.logger.error(
                "Error during the encryption of a message with Enigma. The settings provided do not follow the right format")
            return False
    ...
    def decrypt(self, message, key=0):
       ...
       - a list of 10 tuples of two chars (at most) for the plugboard settings"""
        ...
        if not isinstance(message, str):
            ...
            self.logger.error(
                "Error during the decryption of a message with Enigma. The message must be a string")
            return False

        if not self.checkKeyFormat(key):

            self.logger.error(
                "Error during the decryption of a message with Enigma. The settings provided do not follow the right format")
            return False
