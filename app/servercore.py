...
from app.views.aes_blueprint import AESBlueprint
from app.views.blowfish_blueprint import BlowfishBlueprint
from app.views.caesarcipher_blueprint import CaesarCipherBlueprint
from app.views.des_blueprint import DESBlueprint
from app.views.enigmam3_blueprint import EnigmaM3Blueprint
from app.views.md5_blueprint import MD5Blueprint
from app.views.rsa_blueprint import RSABlueprint
from app.views.sha_blueprint import SHABlueprint
from app.views.vigenerecipher_blueprint import VigenereCipherBlueprint
...
class ServerCore:
    ...
    def __init__(self):
        ...
        fh = logging.handlers.RotatingFileHandler('app/logs/' + __name__ + '.log',
    ...
    def loadBlueprints(self):
        ...
        self.logger.debug("Blueprint added: " + self.aes.blueprintName +
                          ". Algorithm type: " + self.aes.algoType)
        ...
        self.logger.debug("Blueprint added: " + self.blowfish.blueprintName +
                          ". Algorithm type: " + self.blowfish.algoType)
        ...
        self.logger.debug("Blueprint added: " + self.caesarCipher.blueprintName +
                          ". Algorithm type: " + self.caesarCipher.algoType)

        self.des = DESBlueprint()
        self.app.register_blueprint(self.des)
        outdatedAlgos[self.des.blueprintName] = self.des.url
        self.logger.debug("Blueprint added: " + self.des.blueprintName +
                          ". Algorithm type: " + self.des.algoType)
        ...
        self.logger.debug("Blueprint added: " + self.enigmaM3.blueprintName +
                          ". Algorithm type: " + self.enigmaM3.algoType)

        self.md5 = MD5Blueprint()
        self.app.register_blueprint(self.md5)
        hashingAlgos[self.md5.blueprintName] = self.md5.url
        self.logger.debug("Blueprint added: " + self.md5.blueprintName +
                          ". Algorithm type: " + self.md5.algoType)

        self.rsa = RSABlueprint()
        self.app.register_blueprint(self.rsa)
        modernAlgos[self.rsa.blueprintName] = self.rsa.url
        self.logger.debug("Blueprint added: " + self.rsa.blueprintName +
                          ". Algorithm type: " + self.rsa.algoType)

        self.sha = SHABlueprint()
        self.app.register_blueprint(self.sha)
        hashingAlgos[self.sha.blueprintName] = self.sha.url
        self.logger.debug("Blueprint added: " + self.sha.blueprintName +
                          ". Algorithm type: " + self.sha.algoType)
        ...
        self.logger.debug("Blueprint added: " + self.vigenereCipher.blueprintName +
                          ". Algorithm type: " + self.vigenereCipher.algoType)
        ...
        for i in allAlgosList:
            self.allAlgosSorted.append((i, allAlgos[i]))
        for i in historicalAlgosList:
            self.historicalAlgosSorted.append((i, historicalAlgos[i]))
        for i in outdatedAlgosList:
            self.outdatedAlgosSorted.append((i, outdatedAlgos[i]))
        for i in modernAlgosList:
            self.modernAlgosSorted.append((i, modernAlgos[i]))
        for i in hashingAlgosList:
            self.hashingAlgosSorted.append((i, hashingAlgos[i]))
